...
function validateInput(field) {
    ...
    if (field === 'username') {
        errorElement.textContent = value === '' ? "Username is required!" : ""; // لو فاضي طلع ايرورو لو مش فاضي شيل الايرور 
    } else if (field === 'email') {
...
function register() {
    ...
    fields.forEach(field => {
        validateInput(field);
        if (document.getElementById(`${field}-error`).textContent) {             // لو فيه رساله خطا ظهرت هخلي isvalid=false 
            isValid = false;
        }
    });
    ...
    let users = JSON.parse(localStorage.getItem("users")) || []; // هقارن اللي موجود في اللوكل ستواردج 
